...
def extract_manual_input_for_moze(andro_df):
    ...
    accounts = (
        set(andro_df["Expense(Transfer Out)"]) | set(andro_df["Income(Transfer In)"])
    ) - {np.nan}

    # Extract default account amount
    system_df = andro_df[andro_df.Category == "SYSTEM"][
        ["Income(Transfer In)", "Amount"]
    ]
    ...
    categoires_dict = {category: set() for category in categoires}
...
def transformat_andromoney_to_moze(andro_df):
    ...
    # Set 記錄類型
    andro_df.loc[andro_df.is_expense == True, "記錄類型"] = "支出"  # noqa: E712
    andro_df.loc[andro_df.is_income == True, "記錄類型"] = "收入"  # noqa: E712
    andro_df.loc[andro_df.is_transfer == True, "記錄類型"] = "轉帳"  # noqa: E712
    moze_df["記錄類型"] = andro_df["記錄類型"]

    # Update plus/minus of amount
    moze_df.loc[moze_df.記錄類型 == "支出", "金額"] = -moze_df.loc[moze_df.記錄類型 == "支出", "金額"]
    ...
    moze_non_transfer_df = moze_df[moze_df.記錄類型 != "轉帳"]
    moze_non_transfer_df.loc[
        moze_non_transfer_df.Expense.notna(), "帳戶"
    ] = moze_non_transfer_df.loc[moze_non_transfer_df.Expense.notna(), "Expense"]
    moze_non_transfer_df.loc[
        moze_non_transfer_df.Income.notna(), "帳戶"
...
@click.option(
    "--input_file",
    default=DEFAULT_ANDROMONEY_FILENAME,
    help=f"Input Filename (default: {DEFAULT_ANDROMONEY_FILENAME})",
)
@click.option(
    "--output_file",
    default=DEFAULT_MOZE_FILENAME,
...
def extract(ctx):
    ...
    andro_df = load_andromoney_records(input_file)
    accounts, inital_df, projects, categoires_dict = extract_manual_input_for_moze(
        andro_df
    )
